...
class VacancyTest(TestCase):
    ...
    def test_base_vacancy_parser_success(self, mock_parser):
        ...
        response = base_vacancy_parser(self.mock_request, mock_parser, HhVacancy, {})
    ...
    def test_parse_hh_areas(self):
        ...
        self.assertEqual(
            mapping, {"Москва": "Москва", "Подмосковье": "Московская область"}
        )
    ...
    def test_get_city_to_region_mapping_from_cache(
        self, mock_save, mock_get_fixture, mock_exists, mock_get
    ):
    ...
    def test_get_city_to_region_mapping_fetch_hh(self, mock_save, mock_exists, mock_get):
        ...
        mock_save.assert_called_once_with(
            parser.CACHE_FILE, {"Москва": "Москва", "Подмосковье": "Московская область"}
        )
        self.assertEqual(
            mapping, {"Москва": "Москва", "Подмосковье": "Московская область"}
        )
    ...
    @patch("app.services.parser.api_parser.base_parser.os.path.exists")
    @patch("app.services.parser.api_parser.base_parser.save_data")
    def test_get_city_to_region_mapping_fetch_superjob(
        ...
        mock_get.assert_called_once_with(
            "https://api.superjob.ru/2.0/regions/combined/", timeout=10
        )
        mock_save.assert_called_once_with(
            parser.CACHE_FILE,
            {
